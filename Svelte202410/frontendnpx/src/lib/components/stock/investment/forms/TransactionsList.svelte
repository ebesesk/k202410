...
<div class="transactions-summary">
    ...
    <!-- 일반 거래 타입 먼저 표시 -->
    {#each Object.entries($investmentStore?.transactions?.transactions).filter(([key]) => key !== 'active' && key !== 'inactive') as [_key, _data]}
        <div class="asset-section">
            <h4>{_key}</h4>
            {#each Object.entries(_data) as [symbol, data]}
                <div class="list-item transaction-table">
                    <span class="date">{data.id}</span> 
                    <span class="date">{data.date.split('T')[0].slice(5, 10).replace('-', '.')}</span> 
                    {#if data.type === 'DEPOSIT' || data.type === 'WITHDRAWAL'}
                        <span class="type text-gradient">{data.type}</span> 
                        <span class="amount">{formatNumber(data.amount, 0)}</span> 
                        <span class="currency text-gradient">{data.currency}</span> 
                        <span class="debit-account">{data.debit_account.name}</span>
                        <span class="amount text-gradient">|</span>
                        <span class="debit-account">{data.credit_account.name}</span>
                    {:else if data.type === 'EXCHANGE'}
                        ...
                        <span class="currency text-gradient">{data.currency}</span>
                        TO 
                        <span class="amount">{formatNumber(data.quantity, 0)}</span> 
                        <span class="currency text-gradient">{data.note}</span>
                        <span class="amount">{formatNumber(data.exchange_rate, 2)}</span>
                    {:else}
                        <!-- <span class="asset-name name">{data.asset.name}</span> -->
                        <span class="type text-gradient">{data.type.split('_').slice(1).join('_')}</span> 
                        {#if data.type.includes('FEE')}    
                            <span class="quantity">{formatNumber(data.amount, 2)}</span> 
                        {:else}
                            <span class="quantity">{formatNumber(data.amount)}</span> 
                        {/if}
                        <span class="currency text-gradient">{data.currency}</span> 
                        <span class="debit-account">{data.debit_account.name}</span>
                        <span class="amount text-gradient">|</span>
    ...
    <!-- active/inactive 나중에 표시 -->
    




    {#each ['active', 'inactive'] as status}
        {#if $investmentStore?.transactions?.transactions[status]}
            <div class="asset-section">
                {#each Object.entries($investmentStore.transactions.transactions[status]) as [currency, exchanges]}
                    {#if exchanges && Object.keys(exchanges).length > 0}
                        <div class="transaction-currency-group">
                            {#each Object.entries(exchanges) as [exchange, assets]}
                                {#if assets && Object.keys(assets).length > 0 && Object.values(assets).some(asset => asset.name)}
                                    {#each Object.entries(assets) as [symbol, data]}
                                        {#if data && data.name}
                                            <div class="asset-card">
                                                <div class="asset-header" 
                                                    on:click={() => toggleAsset(symbol)}
                                                    class:expanded={expandedAssets[symbol]}>
                                                    <div class="header-content">
                                                        <h4>{data.name} ({symbol})</h4>
                                                        <span class="expand-icon">{expandedAssets[symbol] ? '▼' : '▶'}</span>
                                                    </div>
                                                    <div class="asset-details">
                                                        {#if status === 'active'}
                                                            <span class="amount text-muted">수량:</span>
                                                            <span class="amount">{formatNumber(data.quantity)}</span>
                                                            <span class="amount text-muted">평균단가:</span>
                                                            <span class="amount">{formatNumber(data.price)}</span>
                                                            <span class="amount text-muted">평가금액:</span>
                                                            <span class="amount">{formatNumber(data.amount)}</span>
                                                        {/if}
                                                        <span class="amount text-muted">수수료:</span>
                                                        <span class="amount">{formatNumber(data.fees)}</span>
                                                        <span class="amount text-muted">실현손익:</span>
                                                        <span class={data.pl >= 0 ? 'profit amount' : 'loss amount'}>{formatNumber(data.pl)}</span>
                                                    </div>
                                                </div>
                                                {#if expandedAssets[symbol]}
                                                    <div class="transactions-content" transition:slide>
                                                        {#each data.transactions as transaction}
                                                            <div class="list-item transaction-table">
                                                                {#if !['FEE', 'PROFIT', 'LOSS'].some(type => transaction.type.includes(type))}
                                                                    <span class="date profit-positive">{transaction.id}</span> 
                                                                    <span class="date">{transaction.date.split('T')[0].slice(5, 10).replace('-', '.')}</span> 
                                                                {:else}
                                                                    <span class="date opacity-0">{transaction.date.split('T')[0].slice(5, 10).replace('-', '.')}</span> 
                                                                    <span class="date profit-positive">{transaction.id}</span> 
                                                                {/if}
...
<style>
    ...
    .asset-header h4, .asset-section h4 {
    ...
    .asset-header.expanded .expand-icon {
